...
EVIDENCE_REPORT_RELATIVE_PATHS=(
  "artifacts/readiness/evidence-report.json"
  "artifacts/readiness/evidence-report.md"
)
...
write_report_if_semantically_changed() {
  local target="$1" candidate="$2" kind="$3"

  if [ -f "$target" ]; then
    if node - "$target" "$candidate" "$kind" <<'NODE'
const fs = require('fs');
const [target, candidate, kind] = process.argv.slice(2);

function normalizeJson(file) {
  const clone = JSON.parse(fs.readFileSync(file, 'utf8'));
  delete clone.timestamp;
  return JSON.stringify(clone);
}

function normalizeMarkdown(file) {
  return fs.readFileSync(file, 'utf8').replace(/^\*\*Collected:.*$/m, '**Collected: <ignored>');
}

const normalize = kind === 'json' ? normalizeJson : normalizeMarkdown;
process.exit(normalize(target) === normalize(candidate) ? 0 : 1);
NODE
    then
      rm -f "$candidate"
      return 0
...
EVIDENCE_REPORT_IGNORE_FILE="$(mktemp)"
printf '%s\n' "${EVIDENCE_REPORT_RELATIVE_PATHS[@]}" > "$EVIDENCE_REPORT_IGNORE_FILE"
GIT_STATUS=$(git -C "$PROJECT_ROOT" status --porcelain 2>/dev/null | grep -vFf "$EVIDENCE_REPORT_IGNORE_FILE" || true)
rm -f "$EVIDENCE_REPORT_IGNORE_FILE"
...
record_check "git_tag" "PASS" "Expected release tag: $EXPECTED_TAG"
...
JSON_CANDIDATE="$(mktemp)"
MD_CANDIDATE="$(mktemp)"

cat > "$JSON_CANDIDATE" <<ENDJSON
{
  "timestamp": "$TIMESTAMP",
  "version": "$PKG_VERSION",
...
for check in "${CHECKS[@]}"; do
  ...
  echo "| $NAME | $STATUS | $DETAIL |" >> "$MD_CANDIDATE"
done
...
write_report_if_semantically_changed "$ARTIFACTS_DIR/evidence-report.json" "$JSON_CANDIDATE" json
write_report_if_semantically_changed "$ARTIFACTS_DIR/evidence-report.md" "$MD_CANDIDATE" markdown

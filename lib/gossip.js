 ...
 */
class GossipEngine {
  constructor({ db, keys, pss, fetchFn, ttl = DEFAULT_TTL, coordinator = null }) {
    this.db = db;
    this.keys = keys;
    this.pss = pss;
  ...
  async disseminate(reports) {
    ...
    const activeKeySet = new Set(activePeers.map(p => p.public_key));

    // Coordinator-assisted peer selection: find additional peers
    // from the routing table that are interested in these reports' domains
    let coordinatorPeerKeys = null;
    if (this.coordinator) {
      const allDomains = new Set();
      for (const r of reports) {
        if (r.domain_tags) {
          const tags = Array.isArray(r.domain_tags) ? r.domain_tags : [r.domain_tags];
          tags.forEach(t => allDomains.add(t));
        }
      }
      if (allDomains.size > 0) {
        coordinatorPeerKeys = this.coordinator.selectPeersForReport([...allDomains]);
        if (coordinatorPeerKeys) {
          this.stats.coordinatorRouted++;
        }
      }
    }

    // Build target peer set: active view peers + coordinator-suggested peers
    // that are also in the active view (coordinator narrows, not expands, the set)
    let targetPeers = activePeers;
    if (coordinatorPeerKeys) {
      const coordinatorSet = new Set(coordinatorPeerKeys);
      // Prefer coordinator-selected peers from the active view;
      // always include at least one active peer for overlay health
      const coordinatorMatched = activePeers.filter(p => coordinatorSet.has(p.public_key));
      if (coordinatorMatched.length > 0) {
        targetPeers = coordinatorMatched;
      }
      // If no active peers match coordinator, fall back to full active view
    }

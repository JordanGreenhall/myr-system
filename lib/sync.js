...
function buildBloomFilterFromLocalReports({ db, since, filterBits = 65536, hashCount = 5 }) {
...
async function syncPeer({ db, peer, keys, fetch: fetchFn, relayConfig, replay, recovery = {} }) {
  ...
  async function detectPeerGossipSupport() {
    try {
      const discoveryRes = await fetchWithFallback(peer.peer_url.replace(/\/+$/, '') + '/.well-known/myr-node', {
        method: 'GET',
      });
      if (discoveryRes.status !== 200 || typeof discoveryRes.body !== 'object' || !discoveryRes.body) {
        return false;
      }
      const capabilities = Array.isArray(discoveryRes.body.capabilities)
        ? discoveryRes.body.capabilities
        : [];
      return capabilities.includes('gossip-ihave-iwant') ||
        capabilities.includes('gossip-bloom-anti-entropy');
    } catch {
      return false;
    }
  }
  ...
  async function importReportMeta(reportMeta) {
    ...
    try {
      ...
      logger.error('sync_report_fetch_failed', {
        report_signature: reportMeta.signature,
    ...
    if (report.signature && report.signature !== computedSig) {
      logger.warn('sync_security_hash_mismatch', {
        report_signature: reportMeta.signature,
        peer: peer.operator_name,
      });
      writeTrace(db, {
    ...
    if (operatorSig) {
      ...
      if (!verify(msgToVerify, operatorSig, peer.public_key)) {
        logger.warn('sync_security_invalid_operator_signature', {
          report_signature: reportMeta.signature,
          peer: peer.operator_name,
        });
        writeTrace(db, {
  ...
  async function antiEntropyViaBloom({ since }) {
    const bloomBody = buildBloomFilterFromLocalReports({ db, since });
    const bloomPath = '/myr/sync/bloom';
    const bloomHeaders = makeSignedHeaders({
      method: 'POST',
      urlPath: bloomPath,
      body: bloomBody,
      privateKey: keys.privateKey,
    ...
    let bloomRes;
    try {
      bloomRes = await fetchWithFallback(peer.peer_url.replace(/\/+$/, '') + bloomPath, {
        method: 'POST',
        headers: bloomHeaders,
        body: bloomBody,
      });
    } catch (err) {
      throw new Error(`Network error fetching bloom anti-entropy from ${peer.operator_name}: ${err.message}`);
    }

    if (bloomRes.status === 403 && bloomRes.body?.error?.code === 'peer_not_trusted') {
      return {
        peerNotTrusted: true,
        imported: 0,
        skipped: 0,
    ...
    if (bloomRes.status !== 200) {
      throw new Error(`Failed bloom anti-entropy sync from ${peer.operator_name}: HTTP ${bloomRes.status}`);
    }

    const missingSignatures = Array.isArray(bloomRes.body?.missing_signatures)
      ? bloomRes.body.missing_signatures
      : [];
  ...
  let bloomAttempted = false;
  const canUseBloom = !replay?.from && !replay?.until && await detectPeerGossipSupport();
  if (canUseBloom) {
    bloomAttempted = true;
    try {
      initialWindow = await antiEntropyViaBloom({ since: defaultSince });
    } catch (err) {
      logger.warn('sync_bloom_fallback_to_pull', {
        peer: peer.operator_name,
        error: err.message,
      });
    }
  }
  if (!initialWindow || (bloomAttempted && initialWindow.imported === 0 && initialWindow.failed === 0)) {
    initialWindow = await pullWindow({

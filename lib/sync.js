 ...
 * @returns {Promise<{ imported: number, skipped: number, failed: number, peerNotTrusted?: boolean, peerName: string, relayUsed?: boolean }>}
 */
async function syncPeer({ db, peer, keys, fetch: fetchFn, relayConfig }) {
  ...
  let listRes;
  try {
    listRes = await fetchWithFallback(reportsUrl, { headers: listHeaders });
  } catch (err) {
  ...
  return { imported, skipped, failed, peerName: peer.operator_name, relayUsed };
}
